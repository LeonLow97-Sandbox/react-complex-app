...
function EditPost() {
  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      ...
      case "bodyChange":
        ...
        return;
      case "submitRequest":
        // When sendCount changes, a POST request is made.
    ...
    async function fetchPost() {
      try {
        ...
        if (response.data) {
          ...
          // another user who types the same url cannot enter that edit page.
          if (appState.user.username != response.data.author.username) {
            appDispatch({ type: "flashMessage", value: "You do not have permission to this page" });
  ...
  // for sendCount
  useEffect(() => {
    // if sendCount != 0, it will be true
    if (state.sendCount) {
