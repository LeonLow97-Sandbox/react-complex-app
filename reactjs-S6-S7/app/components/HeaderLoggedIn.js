...
function HeaderLoggedIn(props) {
  ...
  // important to set curly braces because we want to destructure it the object.
  const appDispatch = useContext(DispatchContext);
  const appState = useContext(StateContext);

  function handleLogout() {
    // to remember that we are logged 'out and it does not refresh back into the account
    // setLoggedIn(false);
