...
function Profile() {
  // useParams returns an object /profile/leonlow/123/abc
  const { username } = useParams();
  const appState = useContext(StateContext);
  const [state, setState] = useImmer({
    // checking if follow request has completed
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        // passing token object so server can figure out if we are following this user
  ...
  useEffect(() => {
    if (state.startFollowingRequestCount) {
      // before follow, button is not greyed out
      setState(draft => {
